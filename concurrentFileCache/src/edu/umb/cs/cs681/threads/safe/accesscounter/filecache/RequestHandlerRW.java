...
public class RequestHandlerRW implements Runnable{
	private int from, to;
	private volatile boolean done = false;
	private FileCacheRW cache;
	// private AccessCounter accessC;
	// private ArrayList<java.nio.file.Path> files;
	// ReentrantLock lock = new ReentrantLock();
	// ReentrantReadWriteLock rwLock = new ReentrantReadWriteLock();

	public RequestHandlerRW(int from, int to){
		this.from = from;
		this.to = to; 
		//this.files = new ArrayList<java.nio.file.Path>();
		this.cache = FIFOFileCacheRW.getInstance(); // This might as well have been LRU or LFU
