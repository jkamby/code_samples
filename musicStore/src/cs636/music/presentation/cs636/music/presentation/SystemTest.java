...
public class SystemTest {
	...
	public void run() throws Exception {
		...
		while ((command = getNextCommand(in)) != null) {
			...
			{
				Set<Product> cdList = userService.getProductList(); // TODO call service layer
				if (cdList != null)
					PresentationUtils.displayCDCatlog(cdList, System.out);

			} else if (command.startsWith("gui")) { // get info on user
				String usrEmail = getTokens(command)[1];
				UserData user = userService.getUserInfo(usrEmail); // TODO call service layer
				if (user == null)
					System.out.println("\nNo user with email: " + usrEmail +"\n");
				else
				...
				Product product = userService.getProduct(productCode); // TODO call service layer
				if (product == null)
					System.out.println("\nNo such product\n");
				else
				...
				cart = userService.createCart(); // TODO call service layer
				System.out.println("\n cart created ");
				...
				String params[] = getTokens(command);
				UserData user = userService.getUserInfo(params[1]); // TODO call service layer
				if (user == null)
					System.out.println("\nNo such user\n");
				else {
					InvoiceData invoice = userService.checkout(cart, user.getId()); // TODO call service layer
					System.out.println("\n CDs Ordered..");
				...
				String params[] = getTokens(command);
				Product product = userService.getProduct(params[1]); // TODO call service layer
				if (product == null)
					System.out.println("\nNo such product\n");
				else {
					userService.addItemtoCart(product, cart, 1); // TODO call service layer
					System.out.println("\n Added to Cart..");
				}
			} else if (command.startsWith("setproc")) // process invoice
			{
				int params[] = getIntTokens(command);
				this.adminService.processInvoice(params[1]); // TODO call service layer
			} else if (command.equalsIgnoreCase("gi")) // get list of invoices
			{
				Set<InvoiceData> inv = adminService.getListofInvoices(); // TODO call service layer
				PresentationUtils.displayInvoices(inv, System.out);
			} else if (command.startsWith("gd")) // get list of downloads
			{
				Set<DownloadData> dList = adminService.getListofDownloads(); // TODO call service layer
