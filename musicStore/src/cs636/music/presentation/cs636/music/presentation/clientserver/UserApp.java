...
public class UserApp {
	...
	public UserApp() throws Exception {
		...
		//user = ;
		this.cart = userService.createCart();
	}
	...
	public void handleCatalogPage() throws IOException, ServiceException {
		HashSet<Product> prodlist = new HashSet<Product>();
		Product prod = null;
		while (true) {
			System.out.println("---Catalog Page---");
			prodlist = (HashSet<Product>) this.userService.getProductList();
			...
			prod = this.userService.getProduct(productId); //Product code really!
			if (prod != null) {
				handleProduct(prod);
			}
			else {
				System.out.println(" Can't find the product: " + productId + " !!!");
			}
		}
	}
